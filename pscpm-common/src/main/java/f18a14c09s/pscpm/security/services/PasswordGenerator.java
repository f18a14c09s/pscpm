...
/**
 * Standalone password generator. Note that min length and max length are not
 * properties because they are agruments to at least one overloaded version of
 * generatePassword().
 */
public class PasswordGenerator {
     ...
     * @param specialCharacterAlphabet String. e.g.
     * "`-=[]\\;'./~!@#$%^&*()_+{}|:\"<>?"
    ...
    public static void main(String... args) {
        ...
        for (int i = 0; i < 82; i++) {
            Logger.getLogger(MethodHandles.lookup().lookupClass().getName()).info(new String(gen.generatePassword()));
        }
    }

...
public class SecretsServiceJaxRsAdapter {
    ...
    public byte[] persistSecret(@FormParam("secret_to_persist") String base64EncodedSecret) {
        try {
            Secret<?> secret = IOUtil.deserialize(Base64.getDecoder().decode(base64EncodedSecret));
            return IOUtil.serialize(getSecrets().persistSecret(secret));
    ...
    public byte[] mergeSecret(@FormParam("secret_to_merge") String base64EncodedSecret) {
        try {
            Secret<?> secret = IOUtil.deserialize(Base64.getDecoder().decode(base64EncodedSecret));
            return IOUtil.serialize(getSecrets().mergeSecret(secret));
        } catch (IOException | ClassNotFoundException e) {
            getLog().log(Level.SEVERE, "Failed to refresh secret.", e);
            return new byte[0];
        }
    ...
    public byte[] refresh(@FormParam("secret_to_refresh") String base64EncodedSecret) {
        try {
            Secret<?> secret = IOUtil.deserialize(Base64.getDecoder().decode(base64EncodedSecret));
            return IOUtil.serialize(getSecrets().refresh(secret));
        } catch (IOException | ClassNotFoundException e) {
            getLog().log(Level.SEVERE, "Failed to refresh secret.", e);
            return new byte[0];
        }
    ...
    public byte[] findServers() {
        try {
            return IOUtil.serialize(getSecrets().findServers());
        } catch (IOException e) {
    ...
    public byte[] findCertificates() {
        try {
            return IOUtil.serialize(getSecrets().findCertificates());
        } catch (IOException e) {
    ...
    public byte[] findAvailableServerSecrets() {
        try {
            return IOUtil.serialize(getSecrets().findAvailableServerSecrets());
        } catch (IOException e) {
    ...
    public byte[] findServer(@PathParam("id") long id) {
        try {
            return IOUtil.serialize(getSecrets().findServer(id));
        } catch (IOException e) {
    ...
    public byte[] findCertificate(@PathParam("id") long id) {
        try {
            return IOUtil.serialize(getSecrets().findCertificate(id));
        } catch (IOException e) {

...
public class SecretsServiceJaxRsClientImpl implements ISecretsService {
    ...
    public <EntityClass> Secret<EntityClass> persistSecret(Secret<EntityClass> secret) {
        if (secret != null) {
            ...
            try {
                formData.add("secret_to_persist", Base64.getEncoder().encodeToString(IOUtil.serialize(secret)));
            } catch (IOException e) {
    ...
    public <EntityClass> Secret<EntityClass> mergeSecret(Secret<EntityClass> secret) {
        if (secret != null) {
            MultivaluedMap<String, String> formData = new MultivaluedHashMap<>();
            try {
                formData.add("secret_to_merge", Base64.getEncoder().encodeToString(IOUtil.serialize(secret)));
            } catch (IOException e) {
                _log.log(Level.SEVERE, "Unable to set request parameters.", e);
            }
    ...
    public <EntityClass> Secret<EntityClass> refresh(Secret<EntityClass> secret) {
        if (secret != null) {
            MultivaluedMap<String, String> formData = new MultivaluedHashMap<>();
            try {
                formData.add("secret_to_refresh", Base64.getEncoder().encodeToString(IOUtil.serialize(secret)));
            } catch (IOException e) {
                _log.log(Level.SEVERE, "Unable to set request parameters.", e);
            }
    ...
    @Override
    public <L extends java.util.List<ServerSecret> & java.io.Serializable> L findServers() {
    ...
    @Override
    public <L extends java.util.List<CertificateSecret> & java.io.Serializable> L findCertificates() {
    ...
    @Override
    public <L extends java.util.List<ServerSecret> & java.io.Serializable> L findAvailableServerSecrets() {
